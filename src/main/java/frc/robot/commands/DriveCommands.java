...
public class DriveCommands {
		...
		return Commands.run(
				() -> {
					...
					ChassisSpeeds speeds =
							new ChassisSpeeds(
									drive.getMaximumSpeed().times(linearTranslation.getX()),
									drive.getMaximumSpeed().times(linearTranslation.getY()),
									drive.getMaximumAngularSpeed().times(omega));
	...
	public static Command joystickDriveAtAngle(
		...
		return Commands.run(
						() -> {
							...
							ChassisSpeeds speeds =
									new ChassisSpeeds(
											drive.getMaximumSpeed().times(x),
											drive.getMaximumSpeed().times(y),
											angularVelocity);
	...
	public static Command feedforwardCharacterization(Drive drive) {
		...
		return Commands.sequence(
				...
				Commands.run(
								() -> {
									drive.runCharacterizationLinear(Volts.of(0.0));
								},
								drive)
				...
				Commands.run(
								() -> {
									...
									drive.runCharacterizationLinear(voltage);
						...
						.finallyDo(
								() -> {
									...
									double sumX2 = 0.0;
									for (int i = 0; i < n; i++) {
										sumX += velocitySamples.get(i).baseUnitMagnitude();
										sumY += voltageSamples.get(i);
										sumXY += velocitySamples.get(i).baseUnitMagnitude() * voltageSamples.get(i);
										sumX2 +=
												velocitySamples.get(i).baseUnitMagnitude()
														* velocitySamples.get(i).baseUnitMagnitude();
									}
						...
						Commands.run(
								...
								.finallyDo(
										() -> {
											...
											double wheelRadius =
													(state.gyroDelta * SwerveConstants.DRIVE_BASE_RADIUS.baseUnitMagnitude())
															/ wheelDelta;
