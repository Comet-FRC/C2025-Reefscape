  ...
  public ApriltagVision(ApriltagVisionConsumer consumer, ApriltagVisionIO... io) {
    ...
    for (int i = 0; i < inputs.length; i++) {
      disconnectedAlerts[i] =
          new Alert(
              "ApriltagVision camera " + Integer.toString(i) + " is disconnected.", AlertType.kWarning);
    }
  }
  ...
  public void periodic() {
    ...
    for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
      ...
      for (var observation : inputs[cameraIndex].poseObservations) {
		if (observation.type() == PoseObservationType.INIT)
			io[cameraIndex].initCompleted();
    ...
    Logger.recordOutput(
        "ApriltagVision/Summary/RobotPoses", allRobotPoses.toArray(new Pose3d[allRobotPoses.size()]));
    Logger.recordOutput(
